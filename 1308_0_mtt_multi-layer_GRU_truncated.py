...
rnn_tuple_state = [tf.nn.rnn_cell.LSTMStateTuple(state_per_layer_list[idx][0], state_per_layer_list[idx][1])
...
# rnn_tuple_state_initial = tuple([np.array(np.zeros((2, batch_size, state_size), dtype=np.float32)) for _ in range(num_layers)])
...
def RNN(X, weights, biases):
    ...
    # Forward passes
    outputs, current_state = tf.nn.rnn(cell, X, dtype=tf.float32)
