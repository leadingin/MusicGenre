...
training_files = 14951
valdation_files = 1825
test_files = 4332

# 1 epoch needed iterations
training_iter = int(training_files/batch_size)
validation_iter = int(valdation_files/batch_size)
test_iter = int(test_files/batch_size)

learning_rate = 0.00001
training_iterations = 300 * training_iter # 300 epochs
display_step = 100
state_size = 256
batch_size = 64
num_layers = 4
...
def RNN(X, weights, biases, rnn_tuple_state):
    ...
    # Forward passes
    outputs, current_state = tf.contrib.rnn.static_rnn(cell, X, initial_state=rnn_tuple_state)
...
cross_entropy_losses = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=batchY_placeholder)
...
audio_batch_training, label_batch_training = load_and_shuffle_to_batch_data("data/merge/mtt_scattering_training_filtered.tfrecords", batch_size)
audio_batch_validation, label_batch_validation = load_and_shuffle_to_batch_data("data/merge/mtt_scattering_validation_filtered.tfrecords", batch_size)
audio_batch_test, label_batch_test = load_and_shuffle_to_batch_data("data/merge/mtt_scattering_test_filtered.tfrecords", batch_size)
...
with tf.Session() as sess:
    ...
    for iteration_idx in range(training_iterations):
        ...
        if (iteration_idx + 1) % display_step == 0:
            validation_iterations = validation_iter
    ...
    save_path = saver.save(sess, "model/model_4layer_BNLSTM_256states.ckpt")
