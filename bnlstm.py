...
def batch_norm(x, name_scope, training, epsilon=1e-3, decay=0.999):
    ...
    with tf.variable_scope(name_scope):
        ...
        pop_mean = tf.get_variable('pop_mean', [size], initializer=tf.constant_initializer(value=0.), trainable=False)
        pop_var = tf.get_variable('pop_var', [size], initializer=tf.constant_initializer(value=0.), trainable=False)
