...
with tf.Session() as sess:
    ...
    for iteration_idx in range(training_epochs):
        ...
        for truncated_idx in range(truncated_num):
            ...
            batchX_training = audio_batch_vals_training[:, start_idx:end_idx]
            batchY_training = label_batch_vals_training

            if truncated_idx == (truncated_num-1):

                _train_step, _total_loss, _current_state = sess.run([train_step, total_loss, current_state], feed_dict={batchX_placeholder: batchX_training,
                                                 batchY_placeholder: batchY_training,
                                                 init_state: _current_state})
            else:
                _current_state = sess.run([current_state],feed_dict={batchX_placeholder: batchX_training,
                                                                               batchY_placeholder: batchY_training,
                                                                               init_state: _current_state})
